...
public abstract class AnvilMenuMixin extends ItemCombinerMenu  {
    ...
    @Inject(at = @At("HEAD"), method = {"createResult", "createResultInternal"})
    ...
    public void mayPickup(Player player, boolean hasItem, CallbackInfoReturnable<Boolean> cir) {
        ...
        if (!player.hasInfiniteMaterials() && player.totalExperience < newCost) {
            if (consistentexpcost$trip && player instanceof ServerPlayer serverPlayer) {
                serverPlayer.sendSystemMessage(Component.literal("You need " + newCost + " EXP (roughly level " + getMinimumLevelForXp(newCost) + "), but you only have " + player.totalExperience + " EXP").withStyle(ChatFormatting.RED));
    ...
    @ModifyConstant(method = {"createResult", "createResultInternal"}, constant = @Constant(intValue = 40))
